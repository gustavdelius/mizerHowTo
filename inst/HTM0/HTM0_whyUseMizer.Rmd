...
library(mizerHowTo)
# remotes::install_github("sizespectrum/mizerExperimental") # if needed
library(mizerExperimental)
library(tidyverse)
library(ggpubr)
# Romain and Gustav: can the be a short cut to the sim object that I calibrated? Ad call it HTM0_sim?

sim <- readRDS(system.file("HTM3/sim_opt_time.RDS", package = "mizerHowTo"))
...
# correct gear params
params@gear_params$l25 = params@species_params$l25
params@gear_params$l50 = params@species_params$l50 # assume the species params numbers are correct
...
More information on how this model was calibrated with historical data can be found in our "mizerHowTo" package [tutorial link HTOM3] and in this [blogpost](https://blog.mizer.sizespectrum.org/posts/2021-08-20-a-5-step-recipe-for-tuning-the-model-steady-state/). 
...
plot_relative_biomass = function(sim0, sim1, ratio = F) {

  # assume sim0 is steady state, sim1 is some kind of variation such as fishing

  fish_sim = apply(sim1@n[as.character(2015:2019),,], c(2,3), mean) # mean of last five years
  unfish_sim = sim0@n[dim(sim0@n)[1],,] # last snapshot of steady state sim
...
```{r}
effort <- melt(getEffort(sim))  %>% filter(time >= 1980)

plot_effort<-ggplot(effort) + 
geom_line(aes(x = time, y = value, colour = gear), size = 1) + 
theme_pubr() +
labs(y="Effort",x="") 
...
ggplot(Brel_scen) + 
  geom_line(aes(x=time,y=value,color=sp), size = 1) + 
