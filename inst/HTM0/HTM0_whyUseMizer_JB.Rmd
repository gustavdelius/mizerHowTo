...
### Example #1. Species Collapse and Recovery in a Heavily Fished Multispecies System
...
All species have been heavily fished over the past 100+ years. 
```{r time series, echo = F}
...
Here we  assess whether some species are collapsed relative to their unfished states. We then examine possible  trajectories of recovery.

The gears  and effort through time in the above model were already setup to be species- specific. We can get the unfished states by re-setting effort = 0 and calculating the steady state.

```{r set fishing, eval = T, echo = F, warning=F}
...
No we can compare the biomasses from this unfished scenario to the previous simulation atthe end of the time series.
```{r set fishing, eval = T, echo = F, warning=F}
...
## Example using the North Sea

Current state with fishing effort inputed from data and using sigmoid gear selectivity 
```{r}
...
LFI <- getProportionOfLargeFish(sim)
MW <- getMeanWeight(sim) # take 10g and up
MMW <- getMeanMaxWeight(sim)
slope <- getCommunitySlope(sim)

plot_dat <- data.frame("time" = dimnames(sim@n)[1], "LFI" = LFI, "MW" = MW,
                       "MMW" = MMW[,"mmw_biomass"], "slope" = slope[,"slope"])

plot_dat <- reshape2::melt(plot_dat)
plot_dat$time <- as.numeric(plot_dat$time)
...
```
 ...
 Now let's change the fisheries effort of one species. Fastest way to do it is to edit the catchability.
 
```{r}
# increasing Cod fishing effort
sim@params@species_params$catchability[12] <- sim@params@species_params$catchability[12]*1.5

params2 <- setParams(sim@params)

sim2 <- project(params2)

plotCalibration(sim2)
```
 
 How are the indicators now?
 
```{r}
LFI <- getProportionOfLargeFish(sim2)
MW <- getMeanWeight(sim2) # take 10g and up
MMW <- getMeanMaxWeight(sim2)
slope <- getCommunitySlope(sim2)

plot_dat <- data.frame("time" = dimnames(sim2@n)[1], "LFI" = LFI, "MW" = MW,
                       "MMW" = MMW[,"mmw_biomass"], "slope" = slope[,"slope"])
...
```
 ...
 > Need to concatene plots to show before/after changing the Cod fishing effort| easier to compare
 
 What happens when we add a new species in the ecosystem?
 
 Adding the Mullet from mizer website but would be cool to add bigger predator, just need to get the right parameters
...
params3 <- addSpecies(params2, species_params)

# by default, interaction matrix of new species is set to 1

```
```{r}
sim3 <- project(params3)
plotCalibration(sim3)
```
 The mullet is dying :(
 
```{r}
LFI <- getProportionOfLargeFish(sim3)
MW <- getMeanWeight(sim3) # take 10g and up
MMW <- getMeanMaxWeight(sim3)
slope <- getCommunitySlope(sim3)

plot_dat <- data.frame("time" = dimnames(sim3@n)[1], "LFI" = LFI, "MW" = MW,
                       "MMW" = MMW[,"mmw_biomass"], "slope" = slope[,"slope"])
...
```
 ...
 Not trusting the indicators for now since the species are not even coexisting
 
 Other example: 
 - recovery
 Get a species to slowly decrease in biomass due to fishing and watch it recover by lowering fishing effort
...
getMeanWeight(sim) # take 10g and up
...
Mizer paper:
one simulation with different gear starting at different times targeting different species
show community metrics such as SSB / Yield and slope / large fish indicator / mean maximum weight / mean weight and total biomass at different time steps





### Bits of code to do fisheries analysis
...
# Fishing mortality (F) is a product of the fishing effort, catchability and selectivity. By default the catchability is 1. For a fully selected species (i.e. selectivity = 1), F is therefore equal to the effort.
# In this example, the fishing mortality of a fully selected species is 1.5 year^-1
# The units of effort are unimportant so long as the product of effort, catchability and selectivity are the same as the desired F (i.e. 'per unit time')
...
no_avg_steps <- 400 # no of time steps to average over
...
height <- 10 # 7 
pointsize <- 11 # 8
#postscript(file = "2.eps", width = width/2.5, height = height/2.5, pointsize=pointsize, horizontal = FALSE, onefile=FALSE, paper='special')
png(filename="Figure2.png", width = width, height = height, units="cm",res=800, pointsize=pointsize)
...
time_to_equib <- 100 # 100 years as we assume the unit of time to be years
...
# As mentioned above fishing mortality (F) is a product of the fishing effort, catchability and selectivity. By default the catchability is 1. For a fully selected species (i.e. selectivity = 1), F is therefore equal to the effort.
# The units of effort are unimportant so long as the product of effort, catchability and selectivity are the same as the desired F (i.e. 'per unit time')
...
#postscript(file = "Figure3.eps", width = width/2.5, height = height/2.5, pointsize=pointsize, horizontal = FALSE, onefile=FALSE, paper='special')
png(filename="Figure3.png", width = width, height = height, units="cm",res=800, pointsize=pointsize)

# Figuring out figure panel heights - bit fiddly
rel_heights <- c(0.7,rep(0.5,8),0.5,1,1.1) # rel heights of panels
...
plot(x = 1:project_time, y=1:project_time, type="n", ylim=c(0,max(fishing_effort)), xlab="", ylab="", xaxt="n")
mtext(expression(Effort~(y^{-1})), side=2, line=3, cex=0.6, adj=0.5)
...
add_effort_lines()
#legend(x="bottomright", legend = gear_names, lty=gear_lty, cex=legend_txt_cex, pt.lwd=leg_line, seg.len=seg_len, bty=leg_bty, box.lwd=leg_box_lwd, ncol=2)
legend(x="bottomright", legend = c("Pelagic","Beam","Otter","Ind."), lty=gear_lty, cex=legend_txt_cex, pt.lwd=leg_line, seg.len=seg_len, bty=leg_bty, box.lwd=leg_box_lwd, ncol=2)
...
for (gear in gear_names){
    ...
    legend(x="bottomright", legend=species_in_gear, lty=species_lty[species_in_gear], col=species_col[species_in_gear], cex = legend_txt_cex, pt.lwd=leg_line, seg.len = seg_len, bty=leg_bty, box.lwd=leg_box_lwd, ncol=2)
}
...
plot(x = 1:project_time, y=1:project_time, type="n", ylab="", xlab="Years", ylim=ylim)
mtext("Relative metrics", side=2, line=3, cex=0.6, adj=0.5)
...
add_effort_lines()
legend(x="bottomright", legend = c("LFI", "MW", "MMW"), lty=1, col=c(1,2,3), cex = legend_txt_cex, pt.lwd=leg_line, seg.len = seg_len, bty=leg_bty, box.lwd=leg_box_lwd, ncol=2)
...
legend(x="bottomleft", legend=c("Unfished", "Yr 20: Pelagic", "Yr 40: Beam", "Yr 60: Otter", "Yr 80: Industrial"), lty=1, col=cols, cex = legend_txt_cex, pt.lwd=leg_line, seg.len = seg_len, bty=leg_bty, box.lwd=leg_box_lwd, ncol=2)
dev.off()
# Done
#-----------------------------------------------------------------------------
