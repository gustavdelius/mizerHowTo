...
# small edit to the mizerParam object to change the colours used in plots
params_uncalibrated@linecolour[1:12] <- viridis::viridis(dim(params_uncalibrated@species_params)[1]) # colour gradient of asymptotic sizes
```
...
```{r eval = FALSE}
tuneParams(params_uncalibrated)
```
