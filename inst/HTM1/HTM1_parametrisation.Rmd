...
where $t$ is the age. $L_\infty$ represents the asymptotic length and $1/K$ is approximately the age at maturation. Mizer, however, uses two two other parameters, $W_\infty$ and $h$ (`w_inf` in Mizer), where $W_\infty$ represents the asymptotic weight and $h$ the size-scale maximum consumption rate. Fortunately, the two sets of parameters are related, and one can derive $h$ and $W_\infty$ from von Bertalanffy $K$ and $L_\infty$. 
...
$R_{max}$ affects the density-dependent reproduction rate ($RDD$) by limiting the maximum amount of spawn calculated from the density-independent reproduction rate ($RDI$). Let's look at the $RDI/RDD$ ratio to see how strong $R_{max}$ acts on our different species.
