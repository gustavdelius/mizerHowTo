#' @title Erepro calibration app

#' @description Shiny app which focus on tweaking erepro per species and see the
...
shiny_erepro <- function(params)
{
    ...
    server <- function(input, output) {
        output$toggle_erepro <- renderUI({
            n_spec <- length(params_shiny@species_params$species)
            lapply(1:n_spec, function(i) {
                div(
                    paste(params_shiny@species_params$species[i],
                          'Initial Value:',
                          params_shiny@species_params$erepro[i]),
                    numericInput(inputId = paste0('erepro', i),
                                 "erepro:",
                                 value = params_shiny@species_params$erepro[i])
                )
            })
        })
        # reactive expression
        sim <- eventReactive(input$run, {
            n_spec <- length(params_shiny@species_params$species)
            new_erepro <- c(sapply(1:n_spec, function(i) {
                inputName <- paste("erepro", i, sep = "")
                input[[inputName]]
            }))

            shiny_erepro_output <<- new_erepro

            params_shiny@species_params$erepro <- new_erepro
        ...
        observeEvent(input$done, {
            stopApp(sim()@params@species_params$erepro)
        })
